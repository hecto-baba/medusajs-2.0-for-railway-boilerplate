...
/**
 * Form primitives, ported from the Medusa admin dashboard so the vendor panel's
 * forms are built the same way rather than merely styled to look similar.
 *
 * The only deliberate departure is i18n: the dashboard resolves every label
 * through react-i18next, which this panel does not set up, so the few strings
 * this file owns ("optional") are inline.
 */
...
const useFormField = () => {
  ...
  if (!fieldContext) {
    throw new Error("useFormField should be used within a Form.Field")
  }
