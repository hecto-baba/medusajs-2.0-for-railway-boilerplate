...
type CollectionProductsModalProps = {
  open: boolean
...
export const CollectionProductsModal = ({
  open,
  onOpenChange,
  collectionId,
  collectionTitle,
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["vendor-products-for-collection", search],
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      ...
      return manageVendorCollectionProducts(collectionId, {
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({
        queryKey: ["vendor-collection", collectionId],
      })
      onOpenChange(false)
      onSuccess?.()
    },
    onError: (err: any) => {
      toast.error(err.message || "Failed to update collection products")
    },
  })
        ...
        <FocusModal.Header className="flex items-center justify-between border-b p-4">
          <div>
            <FocusModal.Title asChild>
              <Heading level="h2">Edit Products in {collectionTitle}</Heading>
            </FocusModal.Title>
            <FocusModal.Description className="text-ui-fg-subtle text-sm">
              Select which of your products belong to this collection.
